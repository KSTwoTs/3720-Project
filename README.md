...
Postman (optional, for trying the APIs)
...
Open folder TigerTix/ in VS Code.

Open three terminals (Terminal → New Terminal).

Please note that you need to allow temporary bypass using the following command in
Powershell:
...
Events from /api/events are displayed.

Click Buy Ticket to purchase:

The UI shows a status message.

The ticket count refreshes after purchase.

A screen reader announcement is sent via an aria-live region.

Accessibility highlights

Semantic structure (<main>, <section>, <ul>, <h1-3>, <time>).

Skip link and visible focus outlines for keyboard users.

aria-live announcements for purchase results.

9) Concurrency & Consistency

The client-service uses SQL transactions:

BEGIN IMMEDIATE → lock, read tickets, update tickets = tickets - 1, COMMIT.
...
Create a request to POST http://localhost:5001/api/admin/events with headers:
...
Runner → select the request → Data → choose events.csv → Run.
...
Basic test scaffold (App.test.js) for the frontend.
